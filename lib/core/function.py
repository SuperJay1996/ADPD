...
def adpd_train(config, train_loader, model, tmodel, weight, pose_criterion, kd_pose_criterion,
    ...
    for i, (input, target, target_weight, meta) in enumerate(train_loader):
        ...
        if isinstance(outputs, list):
            ...
            kd_loss = (alpha * kd_pose_loss['bi'].mean(-1) - torch.log(alpha)).mean()
            kd_loss += (beta * kd_pose_loss['p'].mean(-1) - torch.log(beta)).mean()
            kd_loss += (theta * kd_pose_loss['b'].mean(-1) - torch.log(theta)).mean()
            
            for output in outputs[:-1]:
                pose_loss += pose_criterion(output, target, target_weight)
                kd_pose_loss = kd_pose_criterion(output, toutput, target, target_weight)

                kd_loss = (alpha * kd_pose_loss['BiKD'].mean(-1) - torch.log(alpha)).mean()
                kd_loss += (beta * kd_pose_loss['KAD'].mean(-1) - torch.log(beta)).mean()
                kd_loss += (theta * kd_pose_loss['NAD'].mean(-1) - torch.log(theta)).mean()
        ...
        pose_losses.update(pose_loss.item(), input.size(0))
        kd_pose_losses.update(kd_loss.item(), input.size(0))
        losses.update(loss.item(), input.size(0))

        BiKD_losses.update((kd_pose_loss['BiKD'].mean()).item(), input.size(0))
        KAD_losses.update((kd_pose_loss['KAD'].mean()).item(), input.size(0))
        NAD_losses.update((kd_pose_loss['NAD'].mean()).item(), input.size(0))
        ...
        if i % config.PRINT_FREQ == 0:
            msg = 'Epoch: [{0}][{1}/{2}]\t' \
                  ...
                  'Accuracy {acc.val:.3f} ({acc.avg:.3f})'.format(
                      ...
                      (alpha.mean()).item(), (beta.mean()).item(), (theta.mean()).item(),
